...
Phase 0 — 기반. Spring Boot 애플리케이션의 기본 실행 환경과 PostgreSQL 영속성 검증 기반 위에 identity 최소 영속성 모델을 마련한 단계다.
...
- `identity_users` 첫 Flyway migration과 최소 JPA persistence model·Repository·PostgreSQL 제약 테스트
...
1. identity 사용자명 정규화와 회원가입 Application 유스케이스를 구현하고 비밀번호 해시 및 중복 가입 테스트를 추가한다.
2. Phase 0 범위의 세션 인증과 사용자 소유권 기반을 구현하고 권한 경계 테스트를 추가한다.
...
- 회원가입·로그인과 인증·인가 흐름은 아직 구현되지 않았다.
